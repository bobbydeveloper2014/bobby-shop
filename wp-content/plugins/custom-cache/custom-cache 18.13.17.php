...
function custom_cache_start_buffering() {
    ...
    ob_start(function ($html) use ($file) {
        // Loại bỏ riêng display: none khỏi style mà không xóa toàn bộ style khác
        $html = preg_replace_callback('/style="([^"]*)"/i', function($matches) {
            $styles = array_filter(array_map('trim', explode(';', $matches[1])), function($style) {
                return !preg_match('/^display\s*:\s*none\s*$/i', trim($style));
            });
            $new_style = implode('; ', $styles);
            return $new_style ? 'style="' . $new_style . '"' : '';
        }, $html);
